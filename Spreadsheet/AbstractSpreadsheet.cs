...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        private void Visit(String start, String name, ISet<String> visited, LinkedList<String> changed)
        {
            ...
            {
                //Names dependents cannot point back to the start - 'name' as causes forever loop.
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                //Check all nonvisted dependents if they point back to start
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            //No circles within given start 'name' dependency paths.
            changed.AddFirst(name); //NOT DONE COMMENTING - TEST METHODS FIRST TO CONFIRM COMMENTS ARE CORRECT - UNSURE WHAT CHANGED IS FULLY!!!
        }
    }
}
